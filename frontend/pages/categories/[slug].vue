<template>
  <div>
    ...
    <Breadcrumbs :breadcrumbs="[
      ...
      { name: category?.data.name || 'Загрузка...' }
    ]" />
    
    <!-- Заголовок страницы -->
    <div class="mb-8">
      <div v-if="category" class="mb-4">
        <h1 class="text-3xl font-bold mb-2">{{ category.data.name }}</h1>
        <p v-if="category.data.description" class="text-gray-600">{{ category.data.description }}</p>
      </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6 mb-8">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-2">
          <label class="block text-sm font-medium text-gray-700 mb-2">Поиск в категории</label>
          <div class="relative">
            <input 
              v-model="searchQuery"
              type="text" 
              :placeholder="`Поиск в ${category?.data.name || 'категории'}...`"
    ...
    <div v-else-if="products.length === 0" class="text-center py-12">
      ...
      <p class="text-gray-600 mb-4">В данной категории пока нет товаров</p>
...
<script setup>
const { getCategory, getProductsByCategory } = useApi()
...
// SEO
const { data: categoryData } = await useLazyAsyncData(`category-${categorySlug}`, async () => {
  const { data, error } = await getCategory(categorySlug)
...
// Динамически обновляем заголовок когда данные загружены
watch(categoryData, (newCategoryData) => {
  if (newCategoryData?.data) {
    useHead({
      title: `${newCategoryData.data.name} - Nuxt Shop`,
      meta: [
        { 
          name: 'description', 
          content: newCategoryData.data.description || `Товары в категории ${newCategoryData.data.name}` 
        }
      ]
    })
  }
...
const { data: productsData, pending, error, refresh } = await useLazyAsyncData(`category-products-${categorySlug}`, async () => {
  const params = {
  ...
  const { data, error: apiError } = await getProductsByCategory(categorySlug, params)
  if (apiError) throw apiError
  return data
}, {
  server: false,
  watch: [currentPage, searchQuery, sortBy, priceFrom, priceTo, inStock]
})

// Вычисляемые свойства
const products = computed(() => productsData.value?.data || [])
const totalProducts = computed(() => productsData.value?.meta?.total || 0)
const totalPages = computed(() => productsData.value?.meta?.last_page || 1)
